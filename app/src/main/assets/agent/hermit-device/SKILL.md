...
For an existing happ, a successful MCP connection already proves that HermitApp's global development service is enabled; never waste a call checking that switch again. Before a write cycle, call `hermit_runtime_status` once. If its `appId` is the target and `launchChannel` is `dev`, write immediately. Otherwise call `hermit_enter_dev_mode` once: Native creates the single development workspace on first use or reuses it later, switches the target to DEV, and opens that happ, optionally at `route`. Do not separately call `hermit_get_dev_status`, `hermit_get_page_state` or `hermit_open_app` merely to repeat these checks. Use `hermit_get_page_state` only when the task actually needs a page snapshot for state restoration. Data, WebView identity and grants remain attached to the existing `appId`. Leaving dev mode switches launches back to the immutable stable release but preserves the development workspace.
...
After the target is active, read only the development metadata and files needed for the requested edit. `hermit_enter_dev_mode`, `hermit_list_dev_files`, `hermit_read_dev_file` and successful write responses all return the current revision, so do not make a redundant status call when one of those results already supplies it. Every mutation uses `expectedDevRevision`; an `E_CONFLICT` means another write won and the caller must read, merge and retry with a new request ID. `hermit_apply_dev_files` and `hermit_sync_dev_changes` atomically create, overwrite, delete, move or rename text files. Use one batch for related edits. Paths are relative and cannot traverse directories or use the reserved `__hermit` segment.
