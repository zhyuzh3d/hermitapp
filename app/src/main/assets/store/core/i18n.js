(() => {
  ...
  const EN = Object.freeze({
    ...
    "三种添加方式都会使用这个选择。":"This choice applies to all three add methods.",
    "应用列表":"App list",
    "收藏":"Favorites",
    "全部":"All",
    ...
    "自定义图标会覆盖 happ 默认图标；移除后恢复默认。添加到手机桌面前请先保存。":"A custom icon overrides the happ default. Removing it restores the default. Save before adding it to the Home screen.",
    "运行方式":"Runtime mode",
    "happ 运行方式":"happ runtime mode",
    "本地运行":"Run locally",
    "线上实时运行":"Run live online",
    "地址与更新":"Addresses and updates",
    "实时与本地挂载地址（liveUrl）":"Live and local mount address (liveUrl)",
    "修改 liveUrl 会让本地代码以该地址所在的网站身份运行，并使用该网站的页面数据。":"Changing liveUrl makes local code run as that website and use its site data.",
    "一键更新地址（updateUrl）":"One-click update address (updateUrl)",
    "更新地址用于检查后续版本；保存后才能检查更新。":"The update address is used to check future versions and takes effect after saving.",
    "检查更新":"Check for updates",
    "当前版本下载地址":"Current version download address",
    "无":"None",
    "重新下载安装":"Download and reinstall",
    ...
    "尚未创建开发工作副本。":"No development workspace has been created.",
    "代码运行通道":"Code channel",
    "运行正式版本":"Run stable version",
    "运行开发副本":"Run development workspace",
    "开发副本只有一份，继承此 happ 的数据与权限。智能体只有在运行开发副本时才能写入文件。":"There is one development workspace. It shares this happ's data and permissions, and an agent can write files only while it is active.",
    "新版本号":"New version code",
    "新版本名称":"New version name",
    "从正式版重建":"Rebuild from stable",
    "导出发布 ZIP":"Export release ZIP",
    ...
    "创建并运行开发副本":"Create and run development workspace",
    "本地代码和线上实时页面都可用。选择后请点底部“保存修改”才会生效。":"Both local code and a live online page are available. Choose one, then tap “Save changes”.",
    "当前只有本地代码；填写 liveUrl 后可选择线上实时运行。":"Only local code is available. Enter liveUrl to enable live online mode.",
    "当前没有本地代码，需要填写 liveUrl 才能线上实时运行。":"No local code is available. Enter liveUrl to run live online.",
    ...
    "线上实时运行需要填写 liveUrl。":"Live online mode requires liveUrl.",
    "允许跨 Origin 网络？":"Allow cross-origin network access?",
    "保存后，此 happ 的脚本、接口和页面可以主动连接 liveUrl Origin 之外的地址，运行时会持续显示提示。":"After saving, this happ's scripts, APIs, and pages can initiate connections outside the liveUrl origin. A notice remains visible while it runs.",
    "保存并允许":"Save and allow",
    "保存后，Hermit 将直接加载 liveUrl，页面代码可随服务器变化。":"Hermit will load liveUrl directly, so page code can change with the server.",
    "保存后，Hermit 将优先运行手机内已验证的代码版本。":"Hermit will prefer the verified on-device code version.",
    "保存运行方式？":"Save runtime mode?",
    "保存并切换":"Save and switch",
    "仍然保存":"Save anyway",
    "应用设置已保存。":"App settings saved.",
    "应用已不存在。":"The app no longer exists.",
    "以后打开此 happ 将运行开发副本。":"This happ will open with its development workspace.",
    "已切换为运行正式版本，开发副本仍保留。":"Switched to the stable version; the development workspace was retained.",
    "从正式版本重建开发副本？":"Rebuild the development workspace from stable?",
    "未发布的开发文件会被正式版本替换，此操作无法撤销。":"Unpublished development files will be replaced by the stable version. This cannot be undone.",
    "重建开发副本":"Rebuild development workspace",
    "开发副本已从当前正式版本重建。":"Development workspace rebuilt from the current stable version.",
    ...
    "正式版本":"Stable version",
    "本次分享 1 小时内有效，二维码已包含六位分享密码。":"This share is valid for one hour. The QR code includes the six-digit password.",
    "让朋友使用 HermitApp 扫描二维码，即可查看信息并确认安装。":"Have your friend scan the QR code with HermitApp to review the details and confirm installation.",
