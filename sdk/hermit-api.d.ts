...
export type HermitPermissionCapability = "speech" | "microphone.record" | "tts.speak" | "location.approximate" | "location.precise" | "sensors.read" | "sensors.steps" | "wifi.scan" | "wifi.connect" | "bluetooth.scan" | "bluetooth.connect" | "infrared.transmit" | "camera.capture" | "camera.torch" | "screen.capture" | "screen.record" | "clipboard.read" | "network" | "notifications";
export type HermitScreenAudioMode = "none" | "system" | "microphone" | "both";
export type HermitScreenStopReason = "page" | "duration" | "bytes" | "projection-revoked" | "host-stop";
export interface HermitScreenAvailability { supported: boolean; systemAudio: boolean; systemAudioUsages: Array<"media" | "game" | "unknown">; microphone: boolean; audioModes: HermitScreenAudioMode[]; segmenting: boolean; maxDurationMs: number; maxBytes: number; maxFrameRate: number; scales: number[]; videoBitRateMin: number; videoBitRateMax: number; audioBitRateMin: number; audioBitRateMax: number; stillFormats: Array<"jpeg" | "png">; stillMaxEdge: number }
export interface HermitScreenSegment { recordingId: string; index: number; logicalFileId: string | null; url?: string; name?: string; mime: string; size?: number; sha256?: string; durationMs: number; width: number; height: number; audio: HermitScreenAudioMode }
export interface HermitScreenRecording { logicalFileId: string | null; url?: string; name?: string; mime: string; size?: number; sha256?: string; index: number; durationMs: number; width: number; height: number; audio: HermitScreenAudioMode; stoppedBy: HermitScreenStopReason; segments: HermitScreenSegment[]; message?: string }
...
export interface HermitApi {
  ...
  screen: {
    availability(): Promise<HermitScreenAvailability>;
    capture(params?: { maxEdge?: number; format?: "jpeg" | "png" }): Promise<{ cancelled: true } | (HermitFile & { cancelled: false; width: number; height: number; capturedAt: number })>;
    startRecording(params?: { audio?: HermitScreenAudioMode; maxDurationMs?: number; maxBytes?: number; segment?: boolean; scale?: number; frameRate?: number; videoBitRate?: number; audioBitRate?: number; name?: string }): Promise<{ cancelled: true } | { recordingId: string; audio: HermitScreenAudioMode; width: number; height: number; segment: boolean; maxDurationMs: number; maxBytes: number; startedAt: number }>;
    stopRecording(params?: { recordingId?: string }): Promise<HermitScreenRecording>;
    cancelRecording(params?: { recordingId?: string }): Promise<{ cancelled: boolean }>;
  };
