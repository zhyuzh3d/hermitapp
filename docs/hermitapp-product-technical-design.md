...
不在 v1 建设浏览器标签页、手机端 npm/Git 构建、独立 APK 生成、多窗口、后台录音定位或传感器采集、任意 Intent/系统插件执行、NFC/无障碍服务。私有仓库、差分包、云发布和动态原生插件留作扩展；普通二维码只识别 HTTP(S) URL，Hermit 设备分享二维码使用独立 `hermit://share` 合同，扫码后仍须显示包信息并由用户确认安装，不能静默执行。蓝牙只提供前台 BLE 扫描与通用 GATT 连接，经典蓝牙配对和设备专属协议仍交给系统设置与 happ 自身。
...
内置 Store 在用户界面显示为“应用库”，避免被理解为公共应用商店。它随 APK 发布，离线可用，管理所有 Web App。首页只有已安装应用、搜索和“添加”；应用卡片提供打开、收藏、桌面图标、设备分享和管理，应用详情提供地址/来源、版本、权限、存储、备份、开发连接和删除。
...
v1.3 采用黑白与中性灰主题，蓝、紫、橙、绿功能图标用于区分动作；卡片、搜索、底栏与状态层使用低对比半透明材质、细边框、内高光和克制阴影，在深浅主题中保持可读性。主导航固定为“收藏、全部、开发、设置、支持”五个等宽入口，图标容器与按钮中心线一致。收藏与全部复用同一应用列表和添加流程：收藏只过滤 `favorite=true`，卡片爱心即时切换。二维码入口对普通 HTTP(S) 内容预填在线网址表单；对 `hermit://share` 则读取临时包信息并打开独立安装确认。
...
添加页面 URL 时，HermitApp 按入口类型处理：以 `.zip` 结尾的地址直接下载并安装；文件名为 `hermit-install.json` 的地址必须读取该清单并下载其中的同 Origin ZIP；普通页面 URL 尝试读取其 Origin 根目录的 `/hermit-install.json`。前两种以及普通页面发现有效清单时都会产生本地 release，可创建开发副本；普通页面没有可用清单时保存 URL 并采用线上实时运行，没有本地代码，不能进入开发模式。显式安装包或清单读取、格式、摘要或包校验失败时明确报错，不能降级成实时网页。裸域名默认 HTTPS，显式 scheme 必须保留；用户明确添加的局域网 HTTP(S) 地址仍属于 URL/线上来源，并单独经过局域网与明文风险确认。
...
设备间分享只分享代码包。A 设备锁定当前正式 `activeReleaseId`；若实例正在运行开发副本，则锁定点击瞬间的 `devRevision`。Native 将快照生成为标准 ZIP，保存到短期文件目录，并在检测到 Wi-Fi、以太网或手机热点局域网地址时启动一小时临时服务。二维码中央显示 happ 图标，没有图标时显示名称首字符；载荷只含协议版本、临时地址和独立六位分享密码。B 扫码后无需输入密码，但必须确认 happ 名称、版本、作者、大小、快照类型和签名状态，随后由现有安装事务下载、校验和原子安装。安装确认中的桌面图标开关默认开启；Android Launcher 是否最终接受固定图标仍由系统决定。A 端同时提供保存 ZIP、系统分享 ZIP 和停止分享。纯线上实时页面没有可锁定的本地代码，不能伪装成可分享安装包。完整实现合同见 [设备间 happ 分享开发计划](../plans/hermitapp-device-happ-sharing-plan.md)。
...
应用库与每个运行中的 happ 使用独立 Android task；每个 task 只有一个 Activity 和一个可见 WebView。主图标无条件请求应用库 task；应用库、桌面快捷方式和通知统一用稳定的 appId document URI 创建或唤回对应 happ task。同一 happ 不重复创建卡片，不同 happ 可同时保留在系统最近任务中。每张卡片通过 `TaskDescription` 请求实例名称与实例图标；未声明图标时生成稳定的名称首字符图标。部分厂商最近任务界面会接受名称却强制显示 APK 图标，此时只能如实回退，不能为动态 happ 预埋有限数量的静态 Activity alias。Intent 只表达打开目标，不能直接授予权限、指定文件路径或调用管理方法。
...
返回键优先处理网页历史；到根页后结束当前 happ task，系统自然回到此前任务。应用库 task 独立保留，不在 happ Activity 内重建第二份应用库。新窗口按导航规则处理，不创建隐藏的额外 WebView。非 HTTP(S) 协议只允许固定的系统用途，并由明确用户操作触发。
...
包内可选 `hermit.json` 只描述已下载代码的身份、展示作者、入口、路由和版本。`author` 是可选署名文本，不是发布者签名或信任凭据。线上 Origin 根目录可选的 `/hermit-install.json` 只描述同源 ZIP 的相对路径和可选 SHA-256；两者职责不同，不得互相替代。本地包没有 `hermit.json` 也可按默认入口安装。
...
| system | language、openSettings | `language` 无需授权，返回 Android 系统首选 BCP-47 语言及有序偏好列表；不读取或覆盖 happ 自己的语言选择 |
...
开发模式开关持久化，用户明确关闭前不会因退到后台、空闲或 Wi-Fi 临时断开而失效；应用进程重建后会自动恢复监听。服务每 15 秒检查 Wi-Fi、手机热点或以太网地址，地址或端口变化时由 HermitUI 提醒用户重新告知智能体；普通蜂窝数据接口不作为可直连地址发布。采用 Host/Origin 防护、错误尝试限速、请求大小限制和有界线程池。六位密码只有一百万种组合，LAN HTTP 也不加密，因此明确仅用于可信局域网，不做公网安全承诺。完整交付与验收见 `plans/hermitapp-unified-dev-workspace-plan.md`。
...
旧的单应用部署会话仍属于 Hermit 前台生命周期。智能体开发服务使用独立的持久开关：切换页面、进入后台、锁屏、空闲或 Wi-Fi 断开都不关闭，应用进程重新启动后按保存的连接模式和端口恢复；只有用户明确关闭开发模式或切换到互斥的旧部署模式时才清除开关。Android 强制停止或进程尚未重新启动期间没有监听进程，这一边界不伪装成系统常驻服务。
...
替换已有实例数据必须由用户在应用库选择目标。数据导入新的 dataGeneration 后用 Registry 条件更新切换指针；对应 happ task 下次恢复时必须先核对 Registry，发现 dataGeneration、release、运行模式或信任版本变化便销毁旧会话并重建。所有旧写操作绑定原 generation，失败保持旧 generation，切换成功后才清理旧数据。开发连接在替换前关闭。
...
| 智能体开发开关误随页面或后台关闭 | 持久化开关与进程启动恢复 | 连续推送不因切页、锁屏、空闲或换网失效；地址变化明确提示 |
