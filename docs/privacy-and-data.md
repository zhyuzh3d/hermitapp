...
Hermit v1 不包含账号、云同步、广告 SDK 或宿主遥测服务。实例注册信息、Hermit 记录、逻辑文件、本地代码、页面授权与短期开发配置默认只保存在设备上。Android 自动备份和设备迁移备份已关闭；需要迁移时由用户在应用库显式导出备份。
...
敏感能力先经过当前 Web App 的 Hermit grant；麦克风、定位和 Android 17 局域网访问等能力还需满足系统权限。一个 Web App 促使 Hermit 获得系统权限，不会自动使其他 Web App 获得对应 Hermit grant。修改在线应用 Origin 或用备份替换数据会提升信任版本并使旧授权失效；Origin 变化还会轮换 WebView Profile。
