...
- 截屏与录屏抓取整个设备屏幕（含其他应用），所以除逐 happ 授权外,每次开始都还要用户当面确认系统投屏弹窗,宿主不缓存也不复用该同意 —— 已授予的 happ 授权只省掉 Hermit 自己的弹窗。一次投屏同意只允许建立一个虚拟显示,因此录制进行中不能同时截屏,两者各自都要一次新的系统同意。录屏的系统声音来自 Android 播放采集,只覆盖媒体、游戏与未知用途,通话/闹钟/通知采不到,被采集应用也可显式排除,DRM 与受保护内容永远不出现;麦克风音轨另需 RECORD_AUDIO。与 `audio.startRecording` 的并发冲突会被显式拒绝。录屏单次默认三分钟、上限 30 分钟,`maxBytes` 是**单个交付文件**的上限(8 MiB 至 256 MiB,文件库单文件硬顶 256 MiB),达到任一上限正常收尾并返回已落盘文件。要录得更久就打开 `segment`,切段由 `MediaRecorder` 自己滚动输出文件、不重启编码器,因此段间不丢帧,音频全程一条连续流按时间窗分段合流。注意文件库是**按应用**算总量的:单应用逻辑文件总量同样只有 256 MiB,所以一个 256 MiB 的成品会占满该应用的全部分额,分段录制实际能录多久由这条总配额决定。录制期间被采集内容旋转时按等比缩放居中呈现并加黑边,不会自动重建编码器。
