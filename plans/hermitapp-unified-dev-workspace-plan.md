...
运行开发工作副本时，在 WebView 内容顶部显示一条 Native 绘制的 3dp 高亮蓝线，建议颜色 `#2997FF`。该线位于页面 DOM 之外、由宿主根布局控制，页面不能通过 CSS、全屏 API或脚本隐藏；不拦截点击，不注入页面，不改变 Bridge API。只有 `launchChannel=dev` 且当前确实加载 dev generation 时显示，正式 release、HermitUI 和支持页面不显示。

不采用“把 Android 状态栏整体改蓝”的方案。不同厂商、沉浸式窗口、状态栏图标明暗模式和全屏页面会造成表现不一致；3dp 原生顶线更稳定，也更准确地表示当前 WebView 加载的是开发代码。UI 自动化和无障碍信息同时暴露 `运行开发副本` 状态，不能只靠视觉色条判断。
...
5. 应用库中的 dev happ 在亮暗主题下都有明显蓝色卡片状态和文字标识；运行 dev 页面时顶部始终有不可由页面隐藏的 3dp 蓝线。
